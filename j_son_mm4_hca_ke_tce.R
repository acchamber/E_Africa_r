...
i_files=c("ec_ppn_ke_jun_SON0516_ctprobs.txt",
          "g5_ppn_ke_jun_SON0516_ctprobs.txt",
          "ec_ppn_ke_jun_SON9316_etprobs.txt",
          "g5_ppn_ke_jun_SON9316_etprobs.txt") 

num_models = 4.0

mmw=c(1.0,1.0,1.0,1.0)

year0=c(2005,2005,1993,1993)

calw=0.5

o_file = "mm4_ppn_ke_jun_SON0516_tce_probs.txt" 

# IMPORTANT 3 INPUT FILES MUST BE ON SAME GRID

year1=2005     # first year of forecast dataset and analysis
year2=2016    # last year of forecast dataset and analysis
ny=year2-year1+1
...
date_lines_fc = grep(pattern='[0-9]{4}-[0-9]{2}', lines_fc)
...
for (year in year1:year2) {
   ...
   for (mdl in 1:num_models) {
      ...
      blw = read.table(fc_file, sep="",  skip=(year-year0[mdl])*(irows*3+6)+4, nrows=irows, header=FALSE )
      b_probs0 = as.matrix(blw)
      b_probs = b_probs0[, -1]
      lats= b_probs0[,1]
      
      abv = read.table(fc_file, sep="",  skip=(year-year0[mdl])*(irows*3+6)+irows*2+8, nrows=irows, header=FALSE )
